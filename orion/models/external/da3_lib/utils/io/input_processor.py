...
class InputProcessor:
    ...
    def __call__(
        self,
        image: List[Union[np.ndarray, Image.Image, str]],
    ...
    # __call__ helpers
    # -----------------------------
    def _resolve_sequential(self, sequential: bool Optional, num_workers: int) -> bool:
    ...
    ) -> tuple[list[np.ndarray Optional] Optional, list[np.ndarray Optional] Optional]:
        if extrinsics is not None and len(extrinsics) != len(image):
            raise ValueError("Length of extrinsics must match images when provided.")
        if intrinsics is not None and len(intrinsics) != len(image):
    ...
    def _run_parallel(
        ...
        exts_list: List[Optional[np.ndarray]],
        ixts_list: List[Optional[np.ndarray]],
        process_res: int,
    ...
    def _unify_batch_shapes(
        ...
        out_intrinsics: list[np.ndarray Optional],
    ) -> tuple[list[torch.Tensor], list[tuple[int, int]], list[np.ndarray Optional]]:
    ...
    def _process_one(
        ...
        *,
        process_res: int,
        process_res_method: str,
    ) -> tuple[torch.Tensor, tuple[int, int], np.ndarray Optional, np.ndarray Optional]:
    ...
    def _load_image(self, img: Union[np.ndarray, Image.Image, str]) -> Image.Image:
        if isinstance(img, str):
            try:
                ...
                raise ValueError(f"Error loading image from path '{img}': {e}")
...
if __name__ == "__main__":
    ...
    def fmt_k_line(K: np.ndarray Optional) -> str:
        if K is None:
            return "None"
    ...
    def show_result(
        tag: str,
        tensor: torch.Tensor,
        Ks_in: Sequence[np.ndarray Optional] Optional = None,
        Ks_out: Sequence[np.ndarray Optional] Optional = None,
    ):
