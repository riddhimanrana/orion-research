#!/usr/bin/env python3
"""Run precision-focused Orion v2 pipeline with stricter filtering.

Based on Gemini validation feedback, this pipeline addresses:
1. Wall hallucinations (higher confidence threshold)
2. Sliver detections (aspect ratio filtering)
3. Track drift during camera pans (stricter spatial gating)
4. Redundant boxes (tighter NMS)

Usage:
    python scripts/run_precision_pipeline.py --video data/examples/test.mp4 --episode precision_test
"""
...
from orion.perception.config import get_yoloworld_precision_config
...
def run_pipeline(video_path: str, episode_id: str, device: str = "cuda") -> dict:
    ...
    logger.info("=" * 80)
    logger.info("ORION v2 PRECISION PIPELINE (Gemini-optimized)")
    logger.info("=" * 80)
    ...
    # Get precision config (higher thresholds, stricter filtering)
    config = get_yoloworld_precision_config()
    ...
    logger.info(f"Max aspect ratio: {config.detection.max_aspect_ratio}")
    logger.info(f"Aspect ratio low-conf threshold: {config.detection.aspect_ratio_lowconf_threshold}")
    logger.info(f"Max bbox area ratio: {config.detection.max_bbox_area_ratio}")
    ...
    build_memory_from_tracks(
        ...
        cosine_threshold=0.65,  # Higher threshold for precision
...
def main():
    parser = argparse.ArgumentParser(description="Run precision Orion v2 pipeline")
