...
# Source dataset location (frames/videos)
AG_SOURCE_ROOT = "dataset/ag"

# Output and results location
AG_DATA_ROOT = "data/ag_50"
GROUND_TRUTH_FILE = os.path.join(AG_DATA_ROOT, "ground_truth_graphs.json")
PREDICTIONS_FILE = os.path.join(AG_DATA_ROOT, "results", "predictions.json")
METRICS_FILE = os.path.join(AG_DATA_ROOT, "results", "metrics.json")
...
def extract_relationships_as_predictions(graph: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for rel in relationships:
        try:
            ...
            # Get first available bbox or default
...
def main():
    ...
    if per_clip_metrics:
        ...
        all_metrics["aggregated"] = {
            ...
            "entities": {"jaccard_similarity": round(avg_entity_jaccard, 4)},
        }
