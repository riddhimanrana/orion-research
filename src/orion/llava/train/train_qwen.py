...
class DataArguments:
    data_path: Optional[List[str]] = field(
        default=None, metadata={"help": "Optional list of paths to the training data."}
    )
...
def preprocess_qwen_2(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            try:
                ...
                print("e")
                print(
                    "skipping sentence due to unrecognized role: {}".format(
                        sentence["from"]
                    )
                )
                continue
    ...
    for conversation, target in zip(conversations, targets):
        ...
        rounds_before = conversation.split(
            conv.sep
        )  # system->user->assistant->user->assistant->user->assistant->user->assistant->Empty
        ...
        for i, rou in enumerate(rounds):
            ...
            assert parts[0].startswith(conv.roles[0]) or parts[0].startswith(
                conv.system
            )
            parts[0] += split_sep
    ...
    def get_sample(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == "pad":
                ...
                or "anyres_max" in self.data_args.image_aspect_ratio
            ):
