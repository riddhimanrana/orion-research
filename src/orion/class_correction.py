...
Automatically corrects YOLO misclassifications using:
1. CLIP verification (does image match the claimed class?)
2. FastVLM description analysis (what does the VLM actually see?)
3. LLM-based class extraction from rich descriptions

This fixes issues like YOLO saying "hair drier" when FastVLM sees "a knob or handle"
...
class ClassCorrector:
    ...
    def _map_to_coco_class(self, text: str) -> Optional[str]:
        """Map a text description to nearest COCO class"""
        ...
        # Common mappings
        mappings = {
            'knob': 'bottle',  # Closest COCO class for small objects
            'handle': 'bottle',
            'dial': 'bottle',
            'button': 'bottle',
        ...
        for entity in entities:
            ...
            if self.should_correct(original_class, description, confidence, clip_verified=False):
                ...
                if corrected_class and corr_confidence > 0.6:
                    ...
                    correction_map[entity['id']] = f"{original_class} → {corrected_class}"
                    corrected_count += 1
...
def correct_tracking_results(
    ...
    if save_corrected and tracking_results.get('video_path'):
        import json
        from pathlib import Path
        
        video_path = Path(tracking_results['video_path'])
        output_path = Path('data/testing') / f"tracking_results_corrected.json"
